...
func (r cfDNSRecord) libdnsRecord(zone string) (libdns.Record, error) {
	...
	case "SRV":
		// Use relative name for parsing service/transport/name
		relativeName := libdns.RelativeName(r.Name, zone)
		parts := strings.SplitN(relativeName, ".", 3)
		if len(parts) < 3 {
			return libdns.SRV{}, fmt.Errorf("name %v does not contain enough fields; expected format: '_service._proto.name'", relativeName)
		}
